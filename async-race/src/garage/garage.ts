...
async function raceFunction(button: HTMLButtonElement, arrCars: Car[]): Promise<void> {    // start race for cars on page to click the race-button
    ...
    currentCars.forEach (async (car) => {
        ...
        if (animationArr.length === currentCars.length) {
            ...
            if ((arrCars.length >= carsOnPage && currentPage === 1) || currentPage < Math.ceil(arrCars.length / carsOnPage)) next?.removeAttribute('disabled');
        }
    })
}
...
async function resetFunction(arrCars: Car[]): Promise<void> {    // cancel animation and stopped cars to click reset-button
    ...
    if (arrCars.length <= carsOnPage || currentPage === Math.ceil(arrCars.length / carsOnPage)) next?.setAttribute('disabled', 'disabled');
}

function createABButtons(wrapper: HTMLDivElement): HTMLButtonElement[] {
    ...
    wrapper.appendChild(bButton);

    return [aButton, bButton];
}

async function aButtonFunction(car: Car, track: HTMLDivElement, carInstance: HTMLDivElement, aButton: HTMLButtonElement, bButton: HTMLButtonElement, i: number): Promise<void> {
    const response = await startStopCar('started', car.id);
    const animationTime = response.distance / response.velocity;
    const animationWidth = track.clientWidth - carInstance.clientWidth;
    const animation = movingCar(carInstance, animationTime, animationWidth);
...
function drawCarWithControls(wrapper: HTMLDivElement, car: Car, i: number): void {
    ...
    const arrButtons = createABButtons(carWithControls);
    const aButton = arrButtons[0];
    const bButton = arrButtons[1];
    ...
    aButton.addEventListener('click', async () => {
        await aButtonFunction(car, track, carInstance, aButton, bButton, i);
    })

    bButton.addEventListener('click', async () => {
        await bButtonFunction(car, aButton, bButton, i);
    })
}
